...
function GroomingExploreItems() {
    
  const card = jsonDataFitness.CardGB;
  console.log(card)
  return (
    <>
        ...
        <div className="productLists">
          <ul className="electricItemLists">
            {card.items.map((item,index) => (
              <Link to={`/Grooming-Explore/${item.ItemId}`} className="remove-style" key={index}>
...
export default GroomingExploreItems
