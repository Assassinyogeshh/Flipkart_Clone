...
function DisplayBuyingItem() {
    ...
    const showClickedImg = (index) => {
        setHoverImg((previousClicked) => {
            const imgUpdaterr = previousClicked.map((value, idxx) =>
                idxx === index ? 'flex' : 'none'
            );
            console.log(imgUpdaterr)
            return imgUpdaterr
        })
            ...
            <div className="BuyingItemImages">
                ...
                <div className="smallItemImages">
                    ...
                    <div className="selectedItemImages">
                        {data.map((imgObj, index) => {
                            ...
                            return (<img src={imgObj} onClick={() => showClickedImg(index)} onMouseOver={() => showHoverImg(index)} alt="cant find images" key={index} />)
                ...
                <div className="bigItemImages">
                    <div className="selctedShowImage" >
                        ...
                        {data.map((imgObj, index) => {
                            return (<img src={imgObj} onClick={ () =>showClickedImg(index)} onMouseOver={() => showHoverImg(index)} alt="" style={{ display: hoverImgStyle.display[index], margin: '-26rem' }} key={index} />)
                        })}
...
export default DisplayBuyingItem
