...
function FitnessExploreItems() {
  // const { CardSF } = useParams();
  const card = jsonDataFitness.CardSF;
        ...
        <div className="productLists">
          <ul className="electricItemLists">
            {card.items.map((item,index) => (
              <Link to={`/Fitness-Explore/${item.ItemId}`} className="remove-style" key={index}>
...
export default FitnessExploreItems;
