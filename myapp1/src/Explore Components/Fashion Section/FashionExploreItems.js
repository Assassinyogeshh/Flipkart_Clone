...
function FashionExploreItems() {
  const card = jsonDataFitness.CardFE;
  console.log(card)
        ...
        <div className="productLists">
          <ul className="electricItemLists">
            {card.items.map((item,index) => (
              <Link to={`/Fashion-Explore/${item.ItemId}`} className="remove-style" key={index}>
...
export default FashionExploreItems;
